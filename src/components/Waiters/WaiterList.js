...
var waiterObj = require('../../Data/Waiters/Waiters.json').waiters;
  ...
  export function WaiterList() {
    ...
    useEffect(() => {
      if(loadDoneRef.current) return;
        ...
        let addBtn = document.getElementById('addWaiterBtn');
        addBtn.addEventListener('click', (e) => {
          e.preventDefault();
          IpcRenderer.invoke('waiter', 'add-waiter').then((result) => {
            console.log(result);
          })
        })
                ...
                <table className="min-w-full border-separate" style={{ borderSpacing: 0 }}>
                  ...
                  <tbody className="bg-white">
                    {waiterObj.map((person, personIdx) => (
                      <tr key={`${person.firstName}${person.lastName}${person.creationDate}`}>
                        <td
